...
struct DateTime
{
    ...
    static DateTime fromString(const std::string& datetime_str) {
        ...
        // Parse the date part
        if (!(ss >> dt.year >> delimiter && delimiter == '-' &&
            ss >> dt.month >> delimiter && delimiter == '-' &&
            ss >> dt.day)) {
            throw std::invalid_argument("Invalid datetime format (date part)");
        }

        // Skip directly to parsing time, ignoring intermediate space
        if (!(ss >> dt.hour >> delimiter && delimiter == ':' &&
            ss >> dt.minute >> delimiter && delimiter == ':' &&
            ss >> dt.second)) {
            throw std::invalid_argument("Invalid datetime format (time part)");
        }

        // Optionally parse fractional seconds
        if (ss.peek() == '.') {
            ss.ignore(); // Skip the dot
            std::string fractional_seconds;
            ss >> fractional_seconds; // Read fractional seconds (if needed, validate here)
        }

        // Ensure no unexpected extra characters
        if (!ss.eof()) {
            throw std::invalid_argument("Invalid datetime format (unexpected extra data)");
        }
