...
void TransferRepository::save(TransferEntity& entity) {
    try {
        ...
        pqxx::result deductRes = txn.exec_params(
            "UPDATE account SET balance = balance - $1 WHERE id = $2 RETURNING id;",
            entity.getAmount(),
            entity.getFromAccountId());
        if (deductRes.empty()) {
            throw std::runtime_error("Failed to deduct balance. Sender account not found.");
        }
        pqxx::result addRes = txn.exec_params(
            "UPDATE account SET balance = balance + $1 WHERE id = $2 RETURNING id;",
            entity.getAmount(),
            entity.getToAccountId());
        if (addRes.empty()) {
            throw std::runtime_error("Failed to add balance. Receiver account not found.");
        }
        pqxx::result res = txn.exec_prepared("save_transfer",
            entity.getFromAccountId(),
            entity.getToAccountId(),
            entity.getAmount());
        if (!res.empty()) {
            ...
            std::string datetime = res[0]["datetime"].c_str();
            std::cout << "Datetime: " << datetime << std::endl;
            entity.setDateTime(DateTime::fromString(datetime));
        }
        else {
            throw std::runtime_error("Failed to insert transfer record.");
        }

        txn.commit();
    }
    catch (const std::exception& e) {
        throw std::runtime_error("Failed to save transfer and update balances: " + std::string(e.what()));
    }
}
