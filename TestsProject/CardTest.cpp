...
void cardTestWrapper(void (*const fn)())
{
...
void testCannotUpdateCardBecomingForInvalidAccount()
{
    ...
    // Try to update the card to be for an invalid account
    CardDTO cardSource2;
    cardSource2.setAccountId(accountResult.getId()+ 63);
    cardSource2.setPin(3434);
    assert_fail(CardService::update(cardResult.getId(), cardSource2));
}
...
void testCannotUpdateCardBecomingDuplicateForAccount()
{
    ...
    // Create a card for the second account
    CardDTO cardSource2;
    cardSource2.setAccountId(accountResult2.getId());
    cardSource2.setPin(4621);
    const CardDTO &cardResult2(CardService::create(cardSource2));
    assert(CardRepository::getById(cardResult2.getId()).has_value());

    // Try to update the second card to become a duplicate for the first account
    CardDTO duplicateSource;
    duplicateSource.setAccountId(accountResult1.getId());
    duplicateSource.setPin(4882);
    assert_fail(CardService::update(cardResult2.getId(), duplicateSource));
}
...
void testCannotUpdateCardWithInvalidPin()
{
    ...
    // Try to update the card to have an invalid pin
    CardDTO cardSource2;
    cardSource2.setAccountId(accountResult.getId());
    cardSource2.setPin(11320);
    assert_fail(CardService::update(cardResult.getId(), cardSource2));
}
